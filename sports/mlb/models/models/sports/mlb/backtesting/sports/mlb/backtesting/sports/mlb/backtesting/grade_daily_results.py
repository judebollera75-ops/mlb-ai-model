    ...
    session.headers.update(
        {
            ...
            "User-Agent": "mlb-ai-model-result-grader/1.0",
        }
    )
...
def get_requested_grade_date() -> date | None:
    ...
    raw_value = os.getenv("MLB_GRADE_DATE")
...
def load_history() -> pd.DataFrame:
    ...
    if not HISTORY_PATH.exists():
        raise FileNotFoundError(
            f"Historical recommendations were not found: {HISTORY_PATH}"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Could not read historical recommendations: {HISTORY_PATH}"
        ) from exc
    ...
    if missing_columns:
        raise ValueError(
            "Historical recommendations are missing columns: "
...
def get_schedule(
    ...
    for date_record in payload.get("dates", []):
        if not isinstance(date_record, dict):
            continue

        date_games = date_record.get("games", [])
...
def game_status(game: dict[str, Any]) -> str:
    ...
    value = detailed_state or abstract_state or ""
...
def game_is_postponed(game: dict[str, Any]) -> bool:
...
def calculate_total_bases(
    ...
    direct_value = batting.get("totalBases")

    if direct_value is not None:
        return safe_number(direct_value)

    hits = safe_number(batting.get("hits"))
    doubles = safe_number(batting.get("doubles"))
    triples = safe_number(batting.get("triples"))
...
def calculate_hitter_fantasy_score(
    ...
    triples = safe_number(batting.get("triples"))
    home_runs = safe_number(batting.get("homeRuns"))
    runs = safe_number(batting.get("runs"))
            ...
            rows.append(
                {
                    ...
                    "player": player_name,
                    "player_key": normalize_player_name(
                        player_name
                    ),
                    "team": team_name,
                    "side": side,
                    ...
                    "hitter_hits": safe_number(
                        batting.get("hits")
                    ),
                    "hitter_total_bases": (
                        calculate_total_bases(batting)
                    ),
                    "hitter_runs": safe_number(
                        batting.get("runs")
                    ),
                    "hitter_rbis": safe_number(
                        batting.get("rbi")
                    ),
                    "hitter_hits_runs_rbis": (
                        safe_number(batting.get("hits"))
                        + safe_number(batting.get("runs"))
                        + safe_number(batting.get("rbi"))
                    ),
                    "hitter_fantasy_score": (
                    ...
                    "pitcher_strikeouts": safe_number(
                        pitching.get("strikeOuts")
                    ),
                    "pitcher_outs": innings_to_outs(
...
def collect_date_stats(
    ...
    games = get_schedule(session, event_date)
    ...
    for game in games:
        ...
        if game_is_postponed(game):
            metadata["postponed_games"] += 1
            continue

        if not game_is_final(game):
            metadata["unfinished_games"] += 1
            continue

        metadata["final_games"] += 1
        ...
        try:
            ...
            print(
                f"Could not download box score "
                f"{game_pk}: {exc}"
            )
            continue
...
def find_matching_stat_row(
    ...
    if date_stats.empty:
        return (
            None,
            "UNRESOLVED",
            "No completed game box scores were available.",
        )

    player_key = history_row.get("player_key", "")

    candidates = date_stats.loc[
        date_stats["player_key"].eq(player_key)
    ].copy()
    ...
    if candidates.empty:
        return (
            None,
            "UNMATCHED_PLAYER",
            "Player was not found in any final box score.",
        )

    market = history_row.get("market", "")
    requested_game_pk = extract_requested_game_pk(
        history_row
    )

    if requested_game_pk is not None:
        game_candidates = candidates.loc[
            candidates["game_pk"].eq(
                requested_game_pk
            )
        ].copy()
        ...
        if game_candidates.empty:
            return (
                None,
                "GAME_ID_MISMATCH",
                "Player was found, but not in the saved game ID.",
            )

        candidates = game_candidates

    if market.startswith("hitter_"):
    ...
    if candidates.empty:
        return (
            None,
            "DID_NOT_APPEAR",
            "Player was listed but did not record the required appearance.",
        )

    game_count = candidates["game_pk"].nunique()

    if game_count > 1 and requested_game_pk is None:
        return (
            None,
            "AMBIGUOUS_DOUBLEHEADER",
            "Player appeared in multiple games and history lacks a game ID.",
        )

    candidates = candidates.sort_values(
        "game_pk"
    ).drop_duplicates(
        subset=["game_pk", "player_key"],
        keep="last",
    )
...
def grade_history_row(
    ...
    market = history_row.get("market", "")
    history_index = int(history_row.name)

    audit = {
        "event_date": history_row.get("event_date"),
        "history_index": history_index,
        "player": history_row.get("player"),
        "market": market,
        "direction": history_row.get("direction"),
        "line": history_row.get("line"),
        "event_id": history_row.get("event_id"),
        "matched_game_pk": pd.NA,
        "actual_result": pd.NA,
        "grading_status": "UNRESOLVED",
        "grading_note": "",
    ...
    if market not in SUPPORTED_MARKETS:
        audit["grading_status"] = "UNSUPPORTED_MARKET"
        audit["grading_note"] = (
            "Automatic grading is not configured for this market."
        )
    ...
    if pd.isna(line):
        audit["grading_status"] = "INVALID_LINE"
        audit["grading_note"] = (
            "The saved recommendation does not have a valid line."
        )
    ...
    if direction not in {"Over", "Under"}:
        audit["grading_status"] = "INVALID_DIRECTION"
        audit["grading_note"] = (
            "The saved direction is not Over or Under."
        )

        return {}, audit

    stat_row, match_status, match_note = (
    ...
    if stat_row is None:
        audit["grading_status"] = match_status
        audit["grading_note"] = match_note
    ...
    if not np.isfinite(actual_result):
        audit["grading_status"] = "MISSING_STAT"
        audit["grading_note"] = (
            "The player matched, but the required statistic was unavailable."
        )
...
def save_audit(audit_rows: list[dict[str, Any]]) -> None:
...
def run_backtest_if_requested() -> None:
    ...
    if should_run not in {"1", "true", "yes"}:
        return

    if not BACKTEST_SCRIPT.exists():
        print(
            "Backtest was requested, but the script "
            f"was not found: {BACKTEST_SCRIPT}"
        )
        return
...
def grade_daily_results() -> pd.DataFrame:
    ...
    requested_date = get_requested_grade_date()
    ...
    print("=" * 72)
    print("MLB AUTOMATIC RESULT GRADER")
    print("=" * 72)
    print(f"History rows: {len(history):,}")
    print(f"Eligible unresolved rows: {len(eligible):,}")
    ...
    if eligible.empty:
        print("No eligible unresolved recommendations were found.")
        save_audit([])
        return history
    ...
    for event_date in unresolved_dates:
        print(f"\nProcessing {event_date.isoformat()}...")

        try:
            date_stats, metadata = collect_date_stats(
                session,
                event_date,
            )
        except requests.RequestException as exc:
            print(
                f"Could not download games for "
                f"{event_date}: {exc}"
            )
            continue
        ...
        print(
            "Games — "
            f"scheduled: {metadata['scheduled_games']}, "
            f"final: {metadata['final_games']}, "
            f"unfinished: {metadata['unfinished_games']}, "
            f"postponed: {metadata['postponed_games']}"
        )

        date_indices = eligible.index[
            eligible["event_date"].eq(event_date)
        ]
        ...
        for history_index in date_indices:
            ...
            updates, audit = grade_history_row(
    ...
    print("=" * 72)
    print(f"Successfully graded: {total_graded:,}")
    print(f"Updated history: {HISTORY_PATH}")
    print(f"Saved grading audit: {AUDIT_PATH}")

    if not audit_frame.empty:
        print("\nGrading status breakdown:")
        print(
            audit_frame["grading_status"]
            .value_counts()
            .to_string()
        )

        graded_rows = audit_frame.loc[
            audit_frame["grading_status"]
            .eq("GRADED")
        ]
    ...
    print(
        f"\nRemaining unresolved history rows: "
